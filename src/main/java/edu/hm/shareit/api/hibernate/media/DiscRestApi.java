package edu.hm.shareit.api.hibernate.media;
...
import edu.hm.shareit.resources.hibernate.media.HibernateMediaServiceResult;
...
public class DiscRestApi{
    ...
    public Response postDisc(Disc disc) {
        HibernateMediaServiceResult result = mediaService.addDisc(disc);
    ...
    public Response updateDisc(Disc disc, @PathParam("barcode") String barcode)  {
        HibernateMediaServiceResult result = mediaService.updateDisc(disc, barcode);
        return Response.ok(result).status(result.getCode()).build();
    }
}
